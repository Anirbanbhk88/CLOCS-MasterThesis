...
def merge_second_batch(batch_list, _unused=False):
    ...
    #print(example_merged['image_idx'][0])
    for key, elems in example_merged.items():
        if key in [
            ...
            #try:
            if key in ['d3_gt_boxes','gt_2d_boxes']:
                arr = []
                for i,elem in enumerate(elems): #each item/elem in elems array is an image/frame
                    #print(f'key {key} --element size {elem.shape}')
                    if elem.shape[0] == 0:
                        elem = np.full((1,elem.shape[1]),-1, dtype=np.float64)
                    arr.append(elem)
                ret[key] = arr #np.array(elems)#  np.stack(elems, axis=0)
            else:
...
def _read_and_prep_v9(info, root_path, num_point_features, prep_func):
    ...
    v_path = pathlib.Path(root_path) / pc_info['velodyne_path']
    v_path = v_path.parent.parent / (
        v_path.parent.stem + "_reduced") / v_path.name
    i_path = pathlib.Path(root_path) / info['image']['image_path']#pathlib.Path(root_path) / info['img_path']
    points = np.fromfile(
        str(v_path), dtype=np.float32,
        count=-1).reshape([-1, num_point_features])
    image_idx = info['image']['image_idx']
    rect = info['calib']['R0_rect'].astype(np.float32)#info['calib/R0_rect'].astype(np.float32)
    Trv2c = info['calib']['Tr_velo_to_cam'].astype(np.float32)#info['calib/Tr_velo_to_cam'].astype(np.float32)
    ...
    input_dict = {
        ...
        'Trv2c': Trv2c,
        'P2': P2,
        'image_shape': np.array(info['image']["image_shape"], dtype=np.int32),#np.array(info["img_shape"], dtype=np.int32),
        'image_idx': image_idx,
        'image_path': info['image']['image_path'],#info['img_path'],
        'images': image_pang,
