        ...
        for n in range(N):
            ...
            if iw > 0:
                ...
                if ih > 0:
                    ...
                    overlaps[ind,2] = -10
                    overlaps[ind,3] = dis_to_lidar_3d[n,0]
                    tensor_index[ind,0] = k #store index of 2D detection
                    tensor_index[ind,1] = n #store index of 3D detection
                    ind = ind+1
            elif k==K-1:
                overlaps[ind,0] = -10
                overlaps[ind,1] = scores_3d[n,0]
...
@numba.jit(nopython=True,parallel=True)
def build_LSTM_stage_training(boxes, grid_boxes, criterion, scores, bbox_type):
    ...
    for k in range(K):
        for m in range(M):
            ...
            #overlap_idx = 0
            #overlap_indx_list = []
            # overlap_indx_2D_list = []
            # overlap_indx_3D_list = []
            for n in range(N):
                ...
                if iw > 0:
                    ...
                    if ih > 0:
                        ...
                        #overlap_indx_list.append(n)
                    # else:
                    #     overlap_indx_list.append(-1)
                # else:
                #     overlap_indx_list.append(-1)
            # if len(overlap_indx_list)>0:
            #     grid_tensor_index.append(overlap_indx_list)
            # else:
            #     grid_tensor_index.append([-1])
    ...
    if ind > ind_max:
        ind_max = ind
    return grid_boxes #, grid_tensor_index

@numba.jit(nopython=True,parallel=True)
def build_object_index_list(boxes, grid_boxes, criterion, bbox_type):
    N = boxes.shape[0] #70400 shape of BBoxes(2D/3D)
    K = grid_boxes.shape[0] #18 rows of grids
    M = grid_boxes.shape[1] #5 cols of grids
    ind=0
    list_grid_index_per_object=[]
    if N == 0:
        list_grid_index_per_object.append(-1)
    else:
        for n in range(N): #for each 2D/3D object check which grid it has max iou and store the grid index in a list
            qbox_area = ((boxes[n, 2] - boxes[n, 0]) *
                            (boxes[n, 3] - boxes[n, 1]))
            max_iou = 0
            ind = 0
            max_overlap_grid_cell_idx = -1
            for k in range(K):
                for m in range(M):
                    iw = (min(boxes[n, 2], grid_boxes[k, m, 2]) -
                        max(boxes[n, 0], grid_boxes[k, m, 0]))
                    if iw > 0:
                        ...
                        if ih > 0:
                            ...
                            if iou > max_iou:
                                max_iou = iou
                                max_overlap_grid_cell_idx = ind
                    ind = ind+1
            list_grid_index_per_object.append(max_overlap_grid_cell_idx)
    return list_grid_index_per_object                   

#@numba.jit(nopython=True,parallel=True)
def add_grid_features_to_sparse_tensor(sparse_overlaps, tensor_indexes, lstm_grid_boxes, grid_tensor_2D_indexes, grid_tensor_3D_indexes, batch_size):
    #lstm_grid_shape = (lstm_grid_boxes.shape[2], lstm_grid_boxes.shape[3]) #get shape of grid (5,18)
    new_sparse_tensors = []
    for sample_idx in range(batch_size):
        # new_sparse_tensor = torch.zeros((900000, 6), device='cuda:0')
        # new_sparse_tensor[:,:] = -1
        tensor_index = tensor_indexes[sample_idx]
        sparse_overlap = sparse_overlaps[sample_idx] # sparse Tensor shape (1, 4, n, k) 
        if tensor_index[0,0] == -1:
            sparse_overlap = torch.cat((sparse_overlap, torch.zeros_like(sparse_overlap)), dim=1)
        else:
            n = sparse_overlap.shape[2]
            k = sparse_overlap.shape[3]
            sparse_overlap = sparse_overlap.view(4, -1) #reshape to (4, n*k) (70400,4)
            lstm_grid_box = lstm_grid_boxes[sample_idx,:,:,:] #lstm grid tensor shape (4,5,18)
            ...
            sparse_overlap = torch.cat([sparse_overlap, features_2D, features_3D], dim=0)
            sparse_overlap = sparse_overlap.view(1, sparse_overlap.shape[0], n, k)
            
        new_sparse_tensors.append(sparse_overlap)
...
def do_coco_style_eval(gt_annos, dt_annos, current_classes, overlap_ranges,
    ...
    overlap_ranges = overlap_ranges.astype(int)
