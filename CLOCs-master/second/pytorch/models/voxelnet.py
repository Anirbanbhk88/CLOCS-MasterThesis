...
class VoxelNet(nn.Module):
    ...
    def train_stage_2(self, example, preds_dict,top_predictions_list):
        ...
        grid_tensor_index_2D_batch = []
        grid_tensor_index_3D_batch = []
        for box_preds, cls_preds, dir_preds, rect, Trv2c, P2, img_idx, a_mask, image_shape, top_predictions, batch_sample_idx in zip(
            ...
            predictions_dict = {
                ...
                "scores": final_scores,
                #"label_preds": label_preds, ##Anirban: added for second eval result
                "image_idx": img_idx,
                #"metadata": image_shape ##Anirban: added for second eval result
            }
            ...
            #box_2d_preds---> 3D detections converted into 2D in image plane/camera CS
            #box_2d_detector ---> 2D detections generated by the 2D detector
            #box_2d_scores ---> scores from 2D detector
            #final_scores ---> sigmoid scores from 3D detector
            #overlaps1 = iou_test ---> iou overlap b/w lidar and camera
            #tensor_index1 = tensor_index ---> indices map of where the 2D and 3D bboxes overlap


            #find iou between the LiDAR(3D) detections and Camera(2D) detections
            ...
            grid_map_3D = se.build_LSTM_stage_training(box_2d_preds.detach().cpu().numpy(), grid_map_3D, -1, 
                                                                         final_scores.detach().cpu().numpy(), bbox_type= '3D')
            grid_list_index_2D_bbox = se.build_object_index_list(box_2d_detector, grid_map_2D, -1,  bbox_type= '2D') 
            grid_list_index_3D_bbox = se.build_object_index_list(box_2d_preds.detach().cpu().numpy(), grid_map_3D, -1,  bbox_type= '3D')
            end = time.time()
                                                                    
            grid_list_index_2D_bbox = np.array(grid_list_index_2D_bbox, dtype=box_2d_preds.detach().cpu().numpy().dtype)
            grid_list_index_3D_bbox = np.array(grid_list_index_3D_bbox, dtype=box_2d_preds.detach().cpu().numpy().dtype)
            ...
            grid_tensor_index_3D_batch.append(torch.LongTensor(grid_list_index_3D_bbox).cuda())
            non_empty_tensor_index_tensor_batch.append(non_empty_tensor_index_tensor.cuda()) #Anirban
            non_empty_iou_test_tensor_batch.append(non_empty_iou_test_tensor.cuda())  #Anirban
    ...
    def create_image_grids(self, batch_image_shape, type):
        ...
        Args:
            batch_image_shape (_type_): shape of the image
        """
