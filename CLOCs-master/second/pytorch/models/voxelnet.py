...
class VoxelNet(nn.Module):
    ...
    #Anirban
    def create_image_grids(batch_image_shape):
        """generate 18x5 image grids each grids of equal size 69px by 75px

        Args:
            batch_image_shape (_type_): shape of the image
        """
        img_height = 375#batch_image_shape[0,0]
        img_width = 1242 #batch_image_shape[0,1]
        grid_width = img_width//18
        grid_height = img_height//5
        left, top, right, bottom = 0, 0, grid_width, grid_height 
        list= [[[(left+ grid_width*i), (top+ grid_height*j), (right+ grid_width*i), (bottom+ grid_height*j)]  for j in range(5)] for i in range(18)]
        grid_tensor = torch.tensor(list)
        print(grid_tensor)
