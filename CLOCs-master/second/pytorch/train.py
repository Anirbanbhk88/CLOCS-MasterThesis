...
from second.pytorch.builder import (box_coder_builder, input_reader_builder,
                                      ...
                                      second_builder)
from second.utils.eval import get_coco_eval_result, get_official_eval_result,bev_box_overlap,d3_box_overlap,add_grid_features_to_sparse_tensor
        ...
        for _ in range(total_loop):
            ...
            for step in range(steps):
                ...
                for i in range(batch_size):
                    ...
                    batch_flags.append(flag) #store the flag for each batch sample

                cls_loss_sum = cls_loss_sum + cls_losses_reduced
                if any(batch_flags): #if any flag of the batch is 1, that means there is a valid sample in a batch, hence compute gradient descent
                    if train_cfg.enable_mixed_precision:
                        loss *= loss_scale
                    cls_losses_reduced.backward()
                    mixed_optimizer.step()
                    mixed_optimizer.zero_grad()
                    convlstm_cell = conv_lstm.cell_list[0]
                    #print(f'convlstm wts: input gate{convlstm_cell.W_ci}, forget gate{convlstm_cell.W_cf}, output gate{convlstm_cell.W_co}')
...
######Anirban- added to save second results############
def predict_to_kitti_label(sec_results,
                          example,
                          class_names,
                          center_limit_range=None,
                          lidar_input=False):
    predictions_dicts = sec_results #net(example)
    limit_range = None
    ...
    for i, preds_dict in enumerate(predictions_dicts):
        ...
        for j in range(box3d_lidar.shape[0]):
            if limit_range is not None:
                if (np.any(box3d_lidar[j, :3] < limit_range[:3])
                    ...
                    continue
            if "bbox" in preds_dict:
                assert "metadata" in preds_dict
                image_shape = preds_dict["metadata"]
            ...
            anno["name"].append(class_names[int(label_preds[0])])
        ...
        annos[-1]["image_index"] = preds_dict["image_idx"]
    return annos
...
def kitti_anno_to_label_file(annos, folder):
    ...
    for anno in annos:
        image_idx = annos[-1]["image_index"]#anno["metadata"]["image"]["image_idx"]
        label_lines = []
...
def _predict_kitti_to_file(net,
    ...
    for i in range(batch_size):
        t_start = time.time()
        #fusion_cls_preds,flag = fusion_layer(fusion_input[i].cuda(),torch_index[i].cuda())
        fusion_cls_preds,flag = fusion_layer(new_fusion_input[i].cuda(),torch_index[i].cuda()) 
        t_end = time.time()
        t_fusion = t_end - t_start
        fusion_cls_preds_reshape = fusion_cls_preds.reshape(1,200,176,2)
        fusion_cls_preds_batch.append(fusion_cls_preds_reshape)
    fusion_cls_preds_reshape = torch.cat(fusion_cls_preds_batch) 
...
def predict_kitti_to_anno(net,
    ...
    for i in range(batch_size):
        ...
        if test_mode==False:
            ...
            d3_gt_boxes = example["d3_gt_boxes"][i]
            if (d3_gt_boxes[0,:] == np.float(-1)).all(): #if d3_gt_boxes.shape[0] == 0:
                target_for_fusion = np.zeros((1,70400,1))
                positives = torch.zeros(1,70400).type(torch.float32).cuda()
                negatives = torch.zeros(1,70400).type(torch.float32).cuda()
                ...
                print(type(d3_gt_boxes))
                d3_gt_boxes_camera = box_torch_ops.box_lidar_to_camera(
                    d3_gt_boxes, example['rect'][i,:], example['Trv2c'][0,:])
                d3_gt_boxes_camera_bev = d3_gt_boxes_camera[:,[0,2,3,5,6]]
                ###### predicted bev boxes
                pred_3d_box = all_3d_output_camera_dict[i]["box3d_camera"]
                pred_bev_box = pred_3d_box[:,[0,2,3,5,6]]
            ...
            cls_losses_reduced = cls_losses.sum()/(example['labels'].shape[0]/batch_size)
...
def evaluate(config_path,
    ...
    if predict_test:
        ...
        result_name = 'eval_results_ObjTracking'
    if result_path is None:
        result_path = model_dir / result_name
    else:
        result_path = pathlib.Path(result_path)
    ...
    for box_preds, cls_preds, dir_preds, rect, Trv2c, P2, img_idx, a_mask in zip(
        ...
        if selected_boxes.shape[0] != 0:
            ...
            if net._use_direction_classifier:
                ...
                opp_labels = (box_preds[..., -1] > 0) ^ dir_labels.to(torch.bool) #commented Anirban
